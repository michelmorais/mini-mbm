...
| `AppIcon.xcassets` (if `GAME_ICON_PNG`) | `<build-dir>/AppIcon.xcassets/` | Generated by CMake — add build dir to `.gitignore` |
...
| Flag | Description |
|---|---|
| `-DGAME_ICON_PNG=/path/to/icon-1024.png` | Single 1024×1024 PNG — CMake generates the `Assets.xcassets` structure automatically |
| `-DGAME_ICON_DIR=/path/to/Assets.xcassets` | Pre-built asset catalog (full `AppIcon.appiconset` set of sizes) |

**`GAME_ICON_PNG` (recommended for most games):**  
Provide a 1024×1024 PNG.  CMake creates
`<build-dir>/AppIcon.xcassets/AppIcon.appiconset/` containing `Contents.json` and a copy
of your PNG, using the Xcode 14+ single-image universal format (requires iOS 14+
deployment target, which matches `Info.plist.in`).
...
Point to an existing `Assets.xcassets` directory that already contains an
`AppIcon.appiconset` folder with all desired sizes and a matching `Contents.json`.
