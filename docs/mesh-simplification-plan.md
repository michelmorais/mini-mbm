...
frame/subset operations. The first UI slice will expose an explicit `Entire mesh` scope, a target
ratio, source-to-target triangle counts, a skeletal quality notice, the simplification report,
`Save As`, and rollback of the last simplification. It must not reuse the current visibility/removal
checkboxes as an implicit target selection. Later backend and UI slices may add `Selected frame` and
`Selected subsets` together, once weight and offset remapping for partial operations is implemented.
