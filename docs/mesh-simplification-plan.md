...
2. Simplify indexed or non-indexed triangles with an explicit error metric and preservation
   options; non-indexed input receives temporary seam-safe adjacency.
...
The projected edge factor `t` is retained by every collapse and applied consistently to positions,
...
canonical weights; frame-global buffers and weights are remapped and validated by every call.
For legacy non-skeletal geometry animation, the editor exposes one selected frame and passes its
one-based index to the native API. Runtime playback selects complete frame buffers discretely, so
the chosen frame may have a different topology while every unselected frame remains unchanged.
For assets that require vertex correspondence, `Apply shared collapses to all frames` uses frame 1
as the reference, requires identical indices and subset ranges, includes every other frame as a
deformation sample in collapse ranking, and reconstructs all frames from the same compact topology.
The editor disables its single-frame combo and virtual-subset grouping while this mode is active;
individual checked subsets still use the shared sequence across all frames. Candidate buffers for
every frame are validated and allocated before the original buffers are replaced.
For non-indexed triangle lists, source and estimated triangle totals use `vertexCount / 3` instead
of displaying zero. The native operation converts a successful result to indexed geometry; the
editor does not expose a separate conversion option.
