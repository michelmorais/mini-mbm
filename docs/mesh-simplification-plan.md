...
area already owns structural frame/subset operations. It exposes an explicit `Entire mesh` scope, a
target ratio, source-to-target triangle counts, a skeletal quality notice, the simplification
report, the existing `Save As` flow, and rollback of the last simplification. It does not reuse the
current visibility/removal checkboxes as an implicit target selection. Later backend and UI slices
may add `Selected frame` and `Selected subsets` together, once weight and offset remapping for
partial operations is implemented.
