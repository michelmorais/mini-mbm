...
## A5. Two `outline.ps`/`outline.vs` bugs — a compile error and a silently reordered uniform

Found together via direct user testing of mesh_debug's ghost outline preview (6.31.7) on
Metal. Neither reproduced on OpenGL ES or DirectX9, for the same underlying reason each
time: those backends bind shader state **by name**; Metal is the only backend that binds
shader state **by position**, so it is the only backend that a naming/ordering mistake in
shared code can silently break.

### Bug 1 — `patchVInStruct` rewrites field names, not just attribute indices

`outline.vs` was hand-authored with `struct VIn { float3 pos [[attribute(0)]]; float3
normal [[attribute(1)]]; };` and a body that read `in.normal`. This failed to compile
(`no member named 'normal' in 'VIn'`) even though the struct in the source clearly declares
a `normal` field.

Root cause: `patchVInStruct()` (§15, "Prewritten VS programs") does not just fix up
attribute *indices* for the mesh's actual `FVF_PROVIDE_BY_ENGINE` — it unconditionally
**replaces the entire `struct VIn {...}` block** with one generated by `vinStructForFVF()`,
which always names the fields `pos` / `nor` / `uv`. Whatever field names the original
prewritten source used are discarded, silently, for every prewritten VS program, every
time. `outline.vs` is the first prewritten VS to reference the normal in its body (`scale.vs`
and `simple texture.vs` only ever touch `pos`/`uv`), so this had no prior chance to surface.

**Rule for new prewritten VS programs:** always write the vertex-input struct and body using
`pos` / `nor` / `uv` — never `normal`, `position`, `texcoord`, or any other name — regardless
of what looks natural for the source being ported from GLSL/HLSL. The struct you write is
discarded and regenerated; only the field names you use in the *body* matter, and they must
match `vinStructForFVF()`'s fixed convention.

### Bug 2 — CFG variable declaration order is not guaranteed by `contents` iteration

After fixing the compile error, the outline rendered with the right general brightness but
a visibly wrong/shifted hue — e.g. a yellow default color came out looking cyan/green.

Root cause: `outline.ps`'s CFG declares `color` (rgb) before `thickness` (float) in the
resource text, but `SHADER_CFG_LOADER::addVariablesFromContents()` (`shader-cfg.cpp`)
iterated `CFG_FROM_MEMORY::contents`, a `std::map<std::string, std::string>` keyed by the
literal cfg key string (e.g. `"[ps-outline.ps][rgb][color]"`). `std::map` iterates in
**key-alphabetical order**, and `"[float][thickness]"` sorts before `"[rgb][color]"` — so
`thickness` was added to the shader's variable list first, ahead of `color`, regardless of
the resource text's actual order.

This directly violates the invariant §15/A2 already document ("fields must appear in CFG
declaration order so byte-offset handles are correct") — the invariant was true of the
*intent*, but nothing enforced it, because the shared cfg parser that feeds `addVar()` never
guaranteed the order it claims to preserve. `BASE_SHADER::addVar` (Metal) packs vars into a
positional float buffer using a running per-stage offset in *add order*: with `thickness`
added first, the packed buffer for `outline.ps` became `[thickness, color.r, color.g,
color.b]` instead of `[color.r, color.g, color.b, thickness]`, rotating every component read
by the fragment shader (`f[0..2]` as "color") by one slot. OpenGL ES / DirectX9 were
unaffected because their `addVar` resolves each variable independently by name
(`glGetUniformLocation`, `D3DXHANDLE`), so add-order never mattered for them.

Fix: `CFG_FROM_MEMORY` now also records `insertionOrder` — a `std::vector<std::string>` of
keys in first-seen order — alongside `contents`, and `addVariablesFromContents()` iterates
`insertionOrder` instead of `contents` directly (`shader-cfg.h` / `shader-cfg.cpp`).

**Rule for new backends and new built-in shaders:** never assume `CFG_FROM_MEMORY::contents`
preserves declaration order — it is a `std::map`, used elsewhere precisely for its
alphabetical, O(log n)-by-exact-key lookup semantics (`getValue()`), and that is
intentional and should not change. Any code that depends on cfg-variable *order* (only the
Metal positional-buffer path does today) must go through `insertionOrder`, and a future
positional-uniform backend (Vulkan push constants, a game console's constant buffer, etc.)
should double-check `addVariablesFromContents()` is still the one iterating it before
relying on declaration order elsewhere.
