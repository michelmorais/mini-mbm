...
| `include/core_mbm/mesh-manager.h` / `MESH_MBM`, `MESH_MBM_DEBUG`, `BUFFER_MESH` | `MESH_MANAGER` cache/fake-release state is behind `Impl`, but mesh file/debug structs still expose large public layouts. The audit shows `MESH_MBM_DEBUG` is heavily coupled to Lua/editor/backend-debug tooling through direct field access, while `MESH_MBM` has a narrower set of external direct reads centered on `infoPhysics` / `infoAnimation`. `BUFFER_MESH` remains directly inspected by renderers and mesh-debug extraction. | These are file-format/editor/debug data contracts, not OS/backend SDK handles. The audit confirms the main risk is source compatibility across Lua/editor/debug paths rather than backend leakage. | High. `MESH_MBM_DEBUG` is broad compatibility surface; `MESH_MBM` is medium-to-high; `BUFFER_MESH` is medium because render paths still read it directly after `getBuffer()`. | Audit is done. First follow-up should be `MESH_MBM` accessor prep only for repeated physics/animation metadata reads. Keep `MESH_MBM_DEBUG` and `BUFFER_MESH` as separate later milestones; do not batch them together. |
...
1. `MESH_MBM` accessor prep only for repeated metadata reads such as `infoPhysics` and `infoAnimation`. Do not move storage yet.
2. `BUFFER_MESH` read-only compatibility audit and accessor boundary sketch. Do not move storage in the same milestone.
3. `MESH_MBM_DEBUG` field-family audit by consumer group: Lua mesh-debug first, tiled editor second, backend-specific debug import/export last.
