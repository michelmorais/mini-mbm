...
| `include/core_mbm/mesh-manager.h` / `MESH_MBM`, `MESH_MBM_DEBUG`, `BUFFER_MESH` | `MESH_MANAGER` cache/fake-release state is behind `Impl`, but mesh file/debug structs still expose large public layouts. `MESH_MBM` now has accessor prep for repeated physics/animation metadata reads through `getPhysicsInfo()`, `getAnimationInfo()`, `getTotalAnimations()`, and `getAnimationHeader(index)`, and the main render/animation call sites use those helpers. `BUFFER_MESH` audit is complete: renderers, animation texture override loops, backend debug extraction, and owner-side mesh construction still inspect `pBufferGL`, `totalSubset`, and `subset` directly. `MESH_MBM_DEBUG` now has the first Lua-focused accessor slice in place for mesh kind, synchronized transform metadata, draw modes, and detail-payload ownership, while raw `buffer` editing and animation/effect list mutation remain direct. | These are file-format/editor/debug data contracts, not OS/backend SDK handles. The main risk is source compatibility across Lua/editor/debug paths rather than backend leakage. `MESH_MBM` is in the accessor-prep stage; `BUFFER_MESH` is at the audit/boundary-sketch stage; `MESH_MBM_DEBUG` is now in the first accessor-prep stage for Lua metadata/detail paths only. | High overall. `MESH_MBM_DEBUG` is still the broadest compatibility surface because `buffer` editing and animation/effect metadata remain direct even after the first Lua-focused accessor slice. `MESH_MBM` is medium after accessor prep. `BUFFER_MESH` stays medium because it is used both as a live render/runtime view and as backend debug-export input. | `MESH_MBM` accessor prep is done for the first metadata slice. `BUFFER_MESH` audit is done. For `MESH_MBM_DEBUG`, the first Lua metadata/detail accessor slice is done; if continued, pick exactly one next slice: raw `buffer` editing helpers or animation/effect metadata helpers, not both together. |
...
1. Optional `MESH_MBM_DEBUG` follow-up for animation/effect metadata helpers (`infoAnimation`, `lsBlendOperation`) only. Do not batch raw `buffer` editing into that milestone.
2. Optional `MESH_MBM_DEBUG` follow-up for raw `buffer` editing helpers only. Keep it separate from animation/effect metadata.
3. Optional `BUFFER_MESH` accessor prep only for narrow read helpers such as render-buffer handle and subset access. Do not move storage in that milestone.
4. Optional `MESH_MBM` follow-up for remaining focused `infoPhysics` writes in shape/build paths only if the extra helper surface is clearly worth it.
