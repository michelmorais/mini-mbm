...
2. Move backend-only render resource structs private first. `RENDER2TARGET_METAL`, `RENDER2TARGET_DIRECTX9`, and `RENDER2TARGET_GLES` are done; remaining examples are `BUFFER_SPECIFIC`, `GLES_PS_VS`, and `D3D_PS_VS`.
