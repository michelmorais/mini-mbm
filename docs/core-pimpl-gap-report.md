...
| Derived render-type headers such as `sprite.h`, `mesh.h`, `font.h`, `particle.h`, `tile.h`, `shape-mesh.h`, `line-mesh.h`, `background.h`, `gif-view.h`, `texture-view.h` | Base `RENDERIZABLE` state is hidden. The first simple texture-like trio is now done: `TEXTURE_VIEW`, `GIF_VIEW`, and `BACKGROUND` private runtime state moved behind `Impl`. `TEXT_DRAW` / `FONT_DRAW` have now had the first accessor-prep slice for Lua-facing alignment/spacing and `FONT_DRAW -> TEXT_DRAW` width/height handoff, but storage remains in place. Other derived classes may still expose gameplay, asset, editor, or render-type-specific fields. | Most remaining state is not an explicit backend/OS dependency. Some is part of long-standing C++/Lua/editor ergonomics. The texture-only classes and `BACKGROUND` were low-risk because the hidden state was private and the public gameplay-facing fields could remain in place. `TEXT_DRAW` / `FONT_DRAW` are different: restore hooks, broader layout internals, and parent/restore behavior are still direct even after the narrow accessor-prep slice. | Medium to high depending on type. `TEXTURE_VIEW`/`GIF_VIEW`/`BACKGROUND` are now low. `TEXT_DRAW`/`FONT_DRAW` drop to medium for the specific alignment/spacing handoff paths, but remain high for a storage move. | Accessor prep has started. If revisited, continue only with similarly narrow cross-owner/Lua-facing prep slices. Do not pair this family with a storage move until the remaining direct restore/layout seams are explicitly reduced first. |
...
1. Optional next `TEXT_DRAW` / `FONT_DRAW` audit: decide whether any additional narrow cross-owner or Lua-facing prep slice exists after alignment/spacing and width/height handoff, without touching restore/layout storage.
