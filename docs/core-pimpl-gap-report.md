...
| `include/core_mbm/mesh-manager.h` / `MESH_MBM`, `MESH_MBM_DEBUG`, `BUFFER_MESH` | `MESH_MANAGER` cache/fake-release state is behind `Impl`, but mesh file/debug structs still expose large public layouts. `MESH_MBM` now has accessor prep for repeated physics/animation metadata reads through `getPhysicsInfo()`, `getAnimationInfo()`, `getTotalAnimations()`, and `getAnimationHeader(index)`, and the main render/animation call sites use those helpers. `BUFFER_MESH` audit is complete: renderers, animation texture override loops, backend debug extraction, and owner-side mesh construction still inspect `pBufferGL`, `totalSubset`, and `subset` directly. `MESH_MBM_DEBUG` now has Lua-focused accessor prep for mesh kind, synchronized transform metadata, draw modes, detail-payload ownership, animation/effect metadata list access, and raw frame/subset lookup helpers, while raw array storage and `BUFFER_MESH_DEBUG` layout remain direct. | These are file-format/editor/debug data contracts, not OS/backend SDK handles. The main risk is source compatibility across Lua/editor/debug paths rather than backend leakage. `MESH_MBM` is in the accessor-prep stage; `BUFFER_MESH` is at the audit/boundary-sketch stage; `MESH_MBM_DEBUG` is now in the accessor-prep stage for Lua metadata/detail, animation/effect, and frame/subset lookup paths, but not for raw geometry storage. | High overall. `MESH_MBM_DEBUG` is still the broadest compatibility surface because raw array storage (`position`, `normal`, `uv`, `indexBuffer`) and public `BUFFER_MESH_DEBUG` layout remain direct even after the helper slices. `MESH_MBM` is medium after accessor prep. `BUFFER_MESH` stays medium because it is used both as a live render/runtime view and as backend debug-export input. | `MESH_MBM` accessor prep is done for the first metadata slice. `BUFFER_MESH` audit is done. For `MESH_MBM_DEBUG`, Lua metadata/detail, animation/effect, and frame/subset lookup helper slices are done; if continued, the next isolated slice should be raw position/normal/uv/index access helpers only. |
...
1. Optional `MESH_MBM_DEBUG` follow-up for raw position/normal/uv/index access helpers only. Keep it separate from the already-done metadata/effect and frame/subset lookup slices.
2. Optional `BUFFER_MESH` accessor prep only for narrow read helpers such as render-buffer handle and subset access. Do not move storage in that milestone.
3. Optional `MESH_MBM` follow-up for remaining focused `infoPhysics` writes in shape/build paths only if the extra helper surface is clearly worth it.
4. Derived render-type header audit by family: simple texture-like types first, mesh/debug/editor-heavy types last.
