...
6. `ANIMATION_MANAGER` restore backup storage is now behind `Impl`; Lua/render/backup current-index reads now use `getIndexAnimation()`, raw current-index compatibility writes use `setIndexAnimation()`, and public animation list/callback state remains pending broader accessor migration.
