...
| `include/core_mbm/mesh-manager.h` / `MESH_MBM`, `MESH_MBM_DEBUG`, `BUFFER_MESH` | `MESH_MANAGER` cache/fake-release state is behind `Impl`, but mesh file/debug structs still expose large public layouts. `MESH_MBM` now has accessor prep for repeated physics/animation metadata reads through `getPhysicsInfo()`, `getAnimationInfo()`, `getTotalAnimations()`, and `getAnimationHeader(index)`, plus narrow physics-write helpers for reset/cube/sphere/cube-complex/triangle writes used by shape builders, debug cloning, and `MESH_MBM` legacy compatibility readers. `BUFFER_MESH` now also has narrow read helpers for render-buffer lookup, loaded-buffer checks, subset count, and subset lookup, and the main render/animation/backend-debug read paths use them. `MESH_MBM_DEBUG` now has Lua-focused accessor prep for mesh kind, synchronized transform metadata, draw modes, detail-payload ownership, animation/effect metadata list access, raw frame/subset lookup helpers, and typed raw geometry-array helpers, while `BUFFER_MESH_DEBUG` storage layout remains direct. The owner-side mesh-debug algorithm review and the `BUFFER_MESH` storage-move review are both done and were intentionally not turned into helper churn. | These are file-format/editor/debug data contracts, not OS/backend SDK handles. The main risk is source compatibility across Lua/editor/debug paths rather than backend leakage. `MESH_MBM` accessor prep now covers the owned-physics write families that were actually worth isolating. The remaining direct physics population is parser-local `INFO_PHYSICS &` work in shared readers and deprecated converter flows, which is intentionally left direct because it does not describe `MESH_MBM` storage ownership. `BUFFER_MESH` is now in accessor-prep for read paths but not storage relocation because construction/factory paths and debug extraction still depend on the aggregate layout; `MESH_MBM_DEBUG` is now in the accessor-prep stage for Lua metadata/detail, animation/effect, frame/subset lookup, and geometry-array access paths, but not for storage relocation. Owner-side edit/validation algorithms remain deliberately direct because they are the editing behavior, not just leaked storage reads. | High overall. `MESH_MBM_DEBUG` is still the broadest compatibility surface because public `BUFFER_MESH_DEBUG` layout and the editing algorithms both depend on raw arrays even after the Lua helper slices. `MESH_MBM` drops to low for the currently targeted owned-physics write paths; the remaining parser-local physics readers are not an object-layout hygiene target. `BUFFER_MESH` stays medium for storage moves because runtime load/factory code and debug extraction still reach directly into both `BUFFER_MESH` and nested `BUFFER_GL` layout. | `MESH_MBM` metadata and owned-physics helper slices are done; stop here unless there is a deliberate redesign of parser/shared physics-reader APIs. `BUFFER_MESH` read-helper slice and storage-move audit are done; revisit only with a larger runtime-mesh builder/debug-extraction redesign. For `MESH_MBM_DEBUG`, Lua metadata/detail, animation/effect, frame/subset lookup, geometry-array helper slices, and the owner-side algorithm audit are done. If revisited, do it only for a more structural debug-mesh editing redesign, not for another isolated helper pass. |
...
| Derived render-type headers such as `sprite.h`, `mesh.h`, `font.h`, `particle.h`, `tile.h`, `shape-mesh.h`, `line-mesh.h`, `background.h`, `gif-view.h`, `texture-view.h` | Base `RENDERIZABLE` state is hidden. The first simple texture-like trio is now done: `TEXTURE_VIEW`, `GIF_VIEW`, and `BACKGROUND` private runtime state moved behind `Impl`. `TEXT_DRAW` / `FONT_DRAW` have now had two accessor-prep slices: Lua-facing alignment/spacing and `FONT_DRAW -> TEXT_DRAW` width/height handoff, plus cross-owner restore/mesh handoff helpers. Storage remains in place. Other derived classes may still expose gameplay, asset, editor, or render-type-specific fields. | Most remaining state is not an explicit backend/OS dependency. Some is part of long-standing C++/Lua/editor ergonomics. The texture-only classes and `BACKGROUND` were low-risk because the hidden state was private and the public gameplay-facing fields could remain in place. `TEXT_DRAW` / `FONT_DRAW` are different: broader layout internals and restore callback storage are still direct even after the narrow prep slices. | Medium to high depending on type. `TEXTURE_VIEW`/`GIF_VIEW`/`BACKGROUND` are now low. `TEXT_DRAW`/`FONT_DRAW` drop further for the current Lua/cross-owner handoff paths, but remain high for a storage move. | Accessor prep is in progress. If revisited, continue only with similarly narrow cross-owner/Lua-facing prep slices. Do not pair this family with a storage move until the remaining direct restore/layout seams are explicitly reduced first. |
...
1. Optional next `TEXT_DRAW` / `FONT_DRAW` audit: decide whether any additional narrow cross-owner or Lua-facing prep slice exists after alignment/spacing, size handoff, and restore/mesh handoff, without touching restore/layout storage.
2. Revisit `BUFFER_MESH` only if a larger runtime-mesh builder/debug-extraction redesign becomes worth the compatibility cost.
3. Revisit `MESH_MBM_DEBUG` only if a larger debug-mesh editing/storage redesign becomes worth the compatibility cost.
4. Revisit parser-local mesh physics readers only if shared loader/serializer API redesign becomes an explicit goal.
