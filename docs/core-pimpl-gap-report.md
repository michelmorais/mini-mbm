...
Milestone 261 implementation note:

- Continued the derived render-type family work with `BACKGROUND`, but intentionally did not batch `TEXT_DRAW`.
- Moved `BACKGROUND` private runtime layout behind `Impl` in `include/render/background.h` and `src/render/background.cpp`, hiding private mesh/texture/buffer/runtime-state members from the public header while keeping public gameplay-facing fields (`bound`, `howFar3d`, `text`, spacing flags) unchanged.
- Left `TEXT_DRAW` deferred. The audit confirmed it is a higher-risk surface because more of its current state is public/editor-facing or tightly coupled to font/layout behavior.

Milestone 262 implementation note:

- Completed the focused `TEXT_DRAW` / `FONT_DRAW` audit and intentionally kept it report-only.
- `TEXT_DRAW` is not a simple "private runtime storage" case. Its current header mixes public layout-facing state (`aligned`, `spaceXCharacter`, `spaceYCharacter`, `aabbMin`, `aabbMax`) with private render/layout state (`mesh`, `widthFirstLetter`, `beginText`, `endText`, `textWithoutSpeciaLetters`) and restore ownership hooks (`onRestoreFont`, `parentFONT_DRAW`).
- `FONT_DRAW` is also tightly coupled to `TEXT_DRAW` internals today. In `src/render/font.cpp`, the add/load/restore paths directly assign `text->mesh`, copy spacing into `text->spaceXCharacter` / `text->spaceYCharacter`, and compute bounding data from `text->beginText` / `text->endText`.
- Lua is an additional compatibility blocker. `src/lua-wrap/render-table/font-lua.cpp` directly reads and writes `TEXT_DRAW::aligned`, `TEXT_DRAW::spaceXCharacter`, and `TEXT_DRAW::spaceYCharacter`, which means even accessor-prep would need a deliberate Lua migration plan instead of a local storage move.
- Decision: do not take `TEXT_DRAW` / `FONT_DRAW` as the next `Impl` milestone. If this family is revisited later, the safe first step is narrower accessor-prep for the cross-owner `FONT_DRAW -> TEXT_DRAW` interactions and the Lua-facing alignment/spacing fields, not a storage relocation.

Milestone 263 implementation note:

- Completed the `MESH_MBM_DEBUG` owner-side algorithm review and intentionally kept it report-only.
- The remaining direct `buffer` usage in `src/core_mbm/mesh-manager.cpp` is concentrated in behavior-heavy editor/build operations such as `calculateNormals()`, `calculateUV()`, `removeSubset()`, `copyBufferFrom()`, `copySubsetFrom()`, `check()`, `centralizeFrame()`, `rotateFrame()`, `scaleFrame()`, `translateFrame()`, `addIndex()`, and `addVertex()`.
- Those methods are not just passive readers of public layout. They mutate raw vertex/index arrays, subset offsets/counts, per-frame allocation, and validation/workaround behavior in one place. Replacing `this->buffer[...]` with thinner helpers would not materially reduce public-header exposure or prepare a meaningful `Impl` move by itself.
- The compatibility surface is also broad. These owner-side operations are called directly from `editor/mesh_debug.lua`, `editor/sprite_maker.lua`, `editor/font_maker.lua`, `editor/particle_editor.lua`, `plugins/tiled/tile_editor.cpp`, and the Lua wrapper in `src/lua-wrap/render-table/mesh-debug-lua.cpp`.
- Decision: do not spend another milestone on owner-side `MESH_MBM_DEBUG` helper churn alone. If this class is revisited later, it should be because we want a more structural redesign of debug-mesh editing storage/ownership, not because a few internal loops still index `buffer` directly.

Milestone 264 implementation note:

- Completed the `BUFFER_MESH` storage-move review and intentionally kept it report-only.
- `BUFFER_MESH` is still constructed and populated directly in the main mesh runtime load path in `MESH_MBM::loadImpl()` and in shape/font factory paths inside `MESH_MANAGER::loadTrueTypeFont()`, `load()`, `loadIndex()`, and `loadDynamicIndex()`. Those paths do not just read fields; they allocate `subset`, assign `totalSubset`, create `pBufferGL`, and thread raw start/count arrays into `BUFFER_GL::loadBuffer(...)` / `loadBufferDynamic(...)`.
- Runtime mutation is also still tied to the same aggregate layout. `MESH_MBM::setTexture()` directly mutates `buffer[indexFrame].subset[indexSubset].texture` and then pushes the texture through `buffer[indexFrame].pBufferGL`.
- Debug extraction from runtime meshes is another blocker. `MESH_MBM_DEBUG::loadDebugFromMemory()` still reaches through `BUFFER_MESH` into `pBufferGL->fvf`, `pBufferGL->totalSubset`, `indexCountIB`, and `vertexCountVB`, which means a `BUFFER_MESH` storage move would immediately spill into a separate `BUFFER_GL` helper/accessor milestone.
- Decision: do not take a `BUFFER_MESH` storage move next. The previous read-helper slice was the useful low-risk part. Any further work here should start only if we are willing to design a broader runtime-mesh builder/debug-extraction abstraction, not a small field-hiding pass.

Milestone 265 implementation note:

- Took the narrow follow-up from the `MESH_MBM` broader physics-write review instead of keeping it audit-only.
- Added the remaining `MESH_MBM`-owned physics append helpers for the currently used shape families: `appendPhysicsSphere()` and `appendPhysicsCubeComplex()`.
- Migrated the matching `MESH_MBM`-owned write paths to those helpers:
  - The circle path in `src/render/shape-mesh.cpp` now uses `resetPhysicsInfo()` plus `appendPhysicsSphere()` instead of reaching directly into `getPhysicsInfo().lsSphere`.
- `MESH_MBM_DEBUG::loadDebugFromMemory()` was still improved on the source side: it now clones from `meshMemory->getPhysicsInfo()` instead of reading `meshMemory->infoPhysics` directly. The destination remains `MESH_MBM_DEBUG::infoPhysics`, which is intentionally a separate debug-mesh contract and not part of the `MESH_MBM` helper surface.
- Intentionally left file parsing helpers such as `read_detail_mesh_section(...)` and legacy-compat readers on `INFO_PHYSICS &` locals. Those are not `MESH_MBM` storage writes and do not need a `MESH_MBM` method just to mirror a local parser buffer.

Milestone 266 implementation note:

- Finished the remaining `MESH_MBM`-owned legacy physics writes in `src/core_mbm/mesh-manager-legacy.cpp`.
- `MESH_MBM::loadLegacyDetailStep()` now appends cube, sphere, and cube-complex bounds through `appendPhysicsCube()`, `appendPhysicsSphere()`, and `appendPhysicsCubeComplex()`.
- `MESH_MBM::readTriangleDetailCompat()` now appends through `appendPhysicsTriangle()` for both current and legacy triangle readers.
- Final audit decision: stop here for `MESH_MBM` physics helpers. The remaining direct physics population is parser-local `INFO_PHYSICS &` work in shared readers and deprecated converter flows (`read_detail_mesh_section(...)`, `fillLegacyPhysicsIfNeeded(...)`). Abstracting those further would not move `MESH_MBM` storage behind a better boundary; it would just wrap temporary parser containers.

Milestone 267 implementation note:

- Started the `TEXT_DRAW` / `FONT_DRAW` accessor-prep work, but kept it strictly on the agreed narrow seam: Lua-facing alignment/spacing and `FONT_DRAW -> TEXT_DRAW` size handoff.
- Added `TEXT_DRAW` accessors for alignment and spacing in `include/render/font.h` / `src/render/font.cpp`: `setAligned()` / `getAligned()`, `setSpaceXCharacter()` / `getSpaceXCharacter()`, and `setSpaceYCharacter()` / `getSpaceYCharacter()`.
- Migrated the Lua-facing alignment and spacing paths in `src/lua-wrap/render-table/font-lua.cpp` to those accessors instead of direct field writes/reads.
- Migrated `FONT_DRAW::fillAnimation()` to set spacing through the new accessors, and `FONT_DRAW::addText(...)` now gets width/height through the existing `TEXT_DRAW::getWidthHeight(...)` API instead of reading `beginText` / `endText` directly for bounding-size handoff.
- Intentionally left storage and the broader render/layout internals untouched. `beginText`, `endText`, restore hooks, and other layout state remain direct inside `TEXT_DRAW` itself; this milestone only reduced the cross-owner/Lua-facing surface.

Milestone 268 implementation note:

- Took the next narrow `TEXT_DRAW` / `FONT_DRAW` accessor-prep slice for cross-owner restore/mesh handoff.
- Added `TEXT_DRAW::setFontMesh()` and `TEXT_DRAW::setRestoreFontContext()` in `include/render/font.h` / `src/render/font.cpp`.
- Constructors and `release()` now use those helpers instead of assigning `mesh`, `onRestoreFont`, and `parentFONT_DRAW` directly.
- `TEXT_DRAW::onRestoreDevice()`, `FONT_DRAW::fillAnimation()`, and `FONT_DRAW::OnRestore(...)` now use the mesh-handoff helper instead of direct `text->mesh` writes.
- Intentionally left the restore callback fields themselves public in the header for compatibility during this prep stage. This milestone reduces cross-owner direct writes, but it does not hide restore/layout storage yet.

Milestone 269 implementation note:

- Completed the follow-up `TEXT_DRAW` / `FONT_DRAW` stop-point audit and intentionally kept it report-only.
- No further narrow accessor-prep slice was worth taking after alignment/spacing, width/height handoff, and restore/mesh handoff.
- The remaining direct `TEXT_DRAW` state falls into two groups:
  - internal render/layout working state such as `beginText`, `endText`, `widthFirstLetter`, and `textWithoutSpeciaLetters`, all used heavily inside `src/render/font.cpp`;
  - compatibility-facing state such as `aabbMin` / `aabbMax` and restore callback context, which is still part of the current C++/Lua/editor behavior surface.
- Decision: stop this family here for now. Revisit `TEXT_DRAW` / `FONT_DRAW` only if a later milestone explicitly accepts a broader compatibility change or a deeper font/layout redesign.

Milestone 270 implementation note:

- Completed the queued `BUFFER_MESH` follow-up audit and intentionally kept it report-only.
- The current narrow helper surface is already in place for the worthwhile non-owner reads: render-buffer lookup, loaded-buffer checks, subset count, and subset lookup.
- The remaining direct `BUFFER_MESH` layout usage is concentrated in areas that are not just "leaked field reads":
  - owner-side runtime builders and load paths in `src/core_mbm/mesh-manager.cpp`, where `pBufferGL`, `subset`, and `totalSubset` are allocated and filled together;
  - backend-specific extraction/serialization paths that need coordinated access to both `BUFFER_MESH` and nested `BUFFER_GL` aggregate state when rebuilding debug/runtime mesh data.
- Decision: do not add another isolated `BUFFER_MESH` helper slice. Any next step here would need to be a deliberate runtime-mesh builder / backend-debug-extraction redesign, not more accessor churn.

Milestone 271 implementation note:

- Completed the queued `MESH_MBM_DEBUG` follow-up audit and intentionally kept it report-only.
- The already-finished narrow helper slices cover the safe read-oriented seams that were worth isolating for this branch: mesh kind, synchronized transform metadata, draw modes, detail-payload ownership, animation/effect metadata lists, frame/subset lookup, and typed raw geometry-array access.
- The remaining direct `MESH_MBM_DEBUG` surface is redesign-shaped rather than accessor-shaped:
  - Lua mesh-debug still directly manipulates debug-physics payloads, header/material flags, and per-frame/per-subset editing state as part of the tool behavior itself;
  - owner-side debug mesh methods still couple storage, validation, mutation, and save/load behavior around `buffer`, `headerMesh`, `infoPhysics`, `infoAnimation`, `extraInfo`, and related state.
- Decision: stop `MESH_MBM_DEBUG` here for strict-PIMPL hygiene. Revisit it only if a broader debug-mesh editing/storage redesign is explicitly in scope.
...
| `include/core_mbm/mesh-manager.h` / `MESH_MBM`, `MESH_MBM_DEBUG`, `BUFFER_MESH` | `MESH_MANAGER` cache/fake-release state is behind `Impl`, but mesh file/debug structs still expose large public layouts. `MESH_MBM` now has accessor prep for repeated physics/animation metadata reads through `getPhysicsInfo()`, `getAnimationInfo()`, `getTotalAnimations()`, and `getAnimationHeader(index)`, plus narrow physics-write helpers for reset/cube/sphere/cube-complex/triangle writes used by shape builders, debug cloning, and `MESH_MBM` legacy compatibility readers. `BUFFER_MESH` now also has the narrow read helpers that were actually justified for non-owner paths: render-buffer lookup, loaded-buffer checks, subset count, and subset lookup, and the main render/animation/backend-debug read paths use them. `MESH_MBM_DEBUG` already received the worthwhile Lua-focused/helper-prep slices for mesh kind, synchronized transform metadata, draw modes, detail-payload ownership, animation/effect metadata lists, raw frame/subset lookup, and typed raw geometry-array helpers, while `BUFFER_MESH_DEBUG` storage layout remains direct. The owner-side mesh-debug algorithm review, the `BUFFER_MESH` storage-move review, the `BUFFER_MESH` follow-up redesign audit, and the final `MESH_MBM_DEBUG` follow-up audit are all done and were intentionally not turned into helper churn. | These are file-format/editor/debug data contracts, not OS/backend SDK handles. The main risk is source compatibility across Lua/editor/debug paths rather than backend leakage. `MESH_MBM` accessor prep now covers the owned-physics write families that were actually worth isolating. The remaining direct physics population is parser-local `INFO_PHYSICS &` work in shared readers and deprecated converter flows, which is intentionally left direct because it does not describe `MESH_MBM` storage ownership. `BUFFER_MESH` should now be treated as stopped at helper-prep for this branch: what remains is owner-side aggregate construction plus backend-specific extraction/serialization, not another narrow field-access seam. `MESH_MBM_DEBUG` should also now be treated as stopped for this branch: what remains is tool behavior, editing/storage mutation, and compatibility-facing debug payload state rather than another clean helper seam. | High overall. `MESH_MBM_DEBUG` remains the broadest compatibility surface because public `BUFFER_MESH_DEBUG` layout and the editing algorithms both depend on raw arrays and direct state even after the helper slices. `MESH_MBM` drops to low for the currently targeted owned-physics write paths; the remaining parser-local physics readers are not an object-layout hygiene target. `BUFFER_MESH` and `MESH_MBM_DEBUG` are both now effectively high for any further hygiene-only work because the remaining work is redesign-shaped rather than accessor-shaped. | `MESH_MBM` metadata and owned-physics helper slices are done; stop here unless there is a deliberate redesign of parser/shared physics-reader APIs. `BUFFER_MESH` read-helper slice, storage-move audit, and redesign follow-up audit are done; revisit only with a larger runtime-mesh builder/backend-debug-extraction redesign. `MESH_MBM_DEBUG` Lua/helper slices, owner-side algorithm audit, and follow-up redesign audit are done; revisit only with a broader debug-mesh editing/storage redesign, not another isolated helper pass. |
...
| Derived render-type headers such as `sprite.h`, `mesh.h`, `font.h`, `particle.h`, `tile.h`, `shape-mesh.h`, `line-mesh.h`, `background.h`, `gif-view.h`, `texture-view.h` | Base `RENDERIZABLE` state is hidden. The first simple texture-like trio is now done: `TEXTURE_VIEW`, `GIF_VIEW`, and `BACKGROUND` private runtime state moved behind `Impl`. `TEXT_DRAW` / `FONT_DRAW` then received the narrow accessor-prep slices that were actually justified: Lua-facing alignment/spacing, `FONT_DRAW -> TEXT_DRAW` width/height handoff, and cross-owner restore/mesh handoff helpers. Storage remains in place. Other derived classes may still expose gameplay, asset, editor, or render-type-specific fields. | Most remaining state is not an explicit backend/OS dependency. Some is part of long-standing C++/Lua/editor ergonomics. The texture-only classes and `BACKGROUND` were low-risk because the hidden state was private and the public gameplay-facing fields could remain in place. `TEXT_DRAW` / `FONT_DRAW` are different: what remains is broader layout internals and compatibility-facing restore/bounds state, not another clean helper seam. | Medium to high depending on type. `TEXTURE_VIEW`/`GIF_VIEW`/`BACKGROUND` are now low. `TEXT_DRAW`/`FONT_DRAW` are lower for the completed Lua/cross-owner handoff paths, but still high for a storage move or broader header cleanup. | Stop `TEXT_DRAW` / `FONT_DRAW` here unless a later milestone explicitly accepts broader compatibility risk. The next safe work in this area is not another isolated helper pass; it would need a more structural font/layout or compatibility redesign. |
...
1. Revisit parser-local mesh physics readers only if shared loader/serializer API redesign becomes an explicit goal.
2. Revisit `TEXT_DRAW` / `FONT_DRAW` only if a broader compatibility decision or font/layout redesign is explicitly accepted; there is no further narrow helper-only slice queued.
3. Revisit `BUFFER_MESH` only if a larger runtime-mesh builder/backend-debug-extraction redesign is explicitly worth the compatibility cost; there is no further helper-only slice queued.
4. Revisit `MESH_MBM_DEBUG` only if a broader debug-mesh editing/storage redesign is explicitly worth the compatibility cost; there is no further helper-only slice queued.
...
36. The next derived render-type family milestone is complete: `BACKGROUND` private runtime layout moved behind `Impl`, while its public gameplay-facing fields stayed in the header and `TEXT_DRAW` was intentionally deferred as the higher-risk remaining texture-like surface.
37. The `TEXT_DRAW` / `FONT_DRAW` audit milestone is complete: no storage move was taken because restore hooks, cross-owner layout/bounds updates, and Lua-facing alignment/spacing fields are still directly coupled; if revisited later, start with accessor prep only.
38. The `MESH_MBM_DEBUG` owner-side algorithm audit milestone is complete: no helper-only follow-up was taken because the remaining direct `buffer` access is concentrated in editing/validation/allocation behavior used broadly by Lua editors and tools; revisit only for a more structural redesign.
39. The `BUFFER_MESH` storage-move audit milestone is complete: no storage move was taken because runtime load/factory paths and debug extraction still depend on the aggregate `pBufferGL`/`subset`/`totalSubset` layout; revisit only with a broader runtime-mesh builder or debug-extraction redesign.
40. The next `MESH_MBM` physics helper slice is complete: `appendPhysicsSphere()` and `appendPhysicsCubeComplex()` exist, the circle path in `shape-mesh.cpp` now uses `resetPhysicsInfo()` plus `appendPhysicsSphere()` instead of direct `lsSphere` writes, and `MESH_MBM_DEBUG::loadDebugFromMemory()` now reads source physics through `meshMemory->getPhysicsInfo()` instead of `meshMemory->infoPhysics`.
41. The final `MESH_MBM` physics audit is complete: legacy `MESH_MBM` compatibility readers now append through the helper API, and the remaining direct physics population in shared/parsing helpers is intentionally left as parser-local `INFO_PHYSICS` work rather than stretched into object-storage accessors.
42. The first `TEXT_DRAW` / `FONT_DRAW` accessor-prep slice is complete: `TEXT_DRAW` now exposes alignment/spacing accessors, Lua font bindings use them for `align` and spacing updates, `FONT_DRAW::fillAnimation()` uses them for spacing propagation, and `FONT_DRAW::addText(...)` now gets text width/height through the existing public API instead of reading `beginText` / `endText` directly.
43. The next `TEXT_DRAW` / `FONT_DRAW` accessor-prep slice is complete: `TEXT_DRAW` now exposes helper methods for mesh and restore-context handoff, constructors/release use them, and `FONT_DRAW` restore/fill paths plus `TEXT_DRAW::onRestoreDevice()` no longer assign `text->mesh` directly.
44. The `TEXT_DRAW` / `FONT_DRAW` stop-point audit is complete: no further narrow helper-only slice remains after the alignment/spacing, width/height, and restore/mesh handoff work, so this family is deferred until a broader compatibility or font/layout redesign is explicitly accepted.
45. The `BUFFER_MESH` follow-up audit is complete: no further helper-only slice remains after the read-helper work, because the remaining direct layout use is owner-side runtime builder/load/save behavior and backend-specific extraction/serialization that depends on the aggregate `BUFFER_MESH` + `BUFFER_GL` layout.
46. The `MESH_MBM_DEBUG` follow-up audit is complete: no further helper-only slice remains after the Lua/helper-prep work, because the remaining direct state is tied to debug-tool behavior, editing/storage mutation, and compatibility-facing debug payload layout rather than another clean accessor seam.
