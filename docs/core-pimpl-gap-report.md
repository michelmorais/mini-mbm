...
| Derived render-type headers such as `sprite.h`, `mesh.h`, `font.h`, `particle.h`, `tile.h`, `shape-mesh.h`, `line-mesh.h`, `background.h`, `gif-view.h`, `texture-view.h` | Base `RENDERIZABLE` state is hidden. The first simple texture-like trio is now done: `TEXTURE_VIEW`, `GIF_VIEW`, and `BACKGROUND` private runtime state moved behind `Impl`. `TEXT_DRAW` / `FONT_DRAW` have now been audited and remain intentionally deferred. Other derived classes may still expose gameplay, asset, editor, or render-type-specific fields. | Most remaining state is not an explicit backend/OS dependency. Some is part of long-standing C++/Lua/editor ergonomics. The texture-only classes and `BACKGROUND` were low-risk because the hidden state was private and the public gameplay-facing fields could remain in place. `TEXT_DRAW` / `FONT_DRAW` are different: restore hooks, Lua field access, and cross-owner layout/bounds updates are still direct. | Medium to high depending on type. `TEXTURE_VIEW`/`GIF_VIEW`/`BACKGROUND` are now low. `TEXT_DRAW`/`FONT_DRAW` remain high for a storage move, though future accessor prep for cross-owner/Lua-facing fields could be medium. | Keep `TEXT_DRAW` / `FONT_DRAW` out of the immediate queue. Revisit only if we want a dedicated accessor-prep milestone for `FONT_DRAW -> TEXT_DRAW` coupling and Lua-facing alignment/spacing fields, without moving storage in the same step. |
...
1. Consider a dedicated accessor-prep milestone for `TEXT_DRAW` / `FONT_DRAW` cross-owner and Lua-facing fields. Do not pair it with a storage move.
