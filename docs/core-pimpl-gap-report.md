...
| Derived render-type headers such as `sprite.h`, `mesh.h`, `font.h`, `particle.h`, `tile.h`, `shape-mesh.h`, `line-mesh.h`, `background.h`, `gif-view.h`, `texture-view.h` | Base `RENDERIZABLE` state is hidden. The first simple texture-like trio is now done: `TEXTURE_VIEW`, `GIF_VIEW`, and `BACKGROUND` private runtime state moved behind `Impl`. Other derived classes may still expose gameplay, asset, editor, or render-type-specific fields. | Most remaining state is not an explicit backend/OS dependency. Some is part of long-standing C++/Lua/editor ergonomics. The texture-only classes and `BACKGROUND` were low-risk because the hidden state was private and the public gameplay-facing fields could remain in place. | Medium to high depending on type. `TEXTURE_VIEW`/`GIF_VIEW`/`BACKGROUND` are now low. `TEXT_DRAW`/`FONT_DRAW` remain higher because they mix public/editor-facing layout state, parent/restore hooks, and text-layout behavior. | Continue by family only when the next type is clearly isolated. After `BACKGROUND`, the next safe step is not an immediate move but a narrower audit of whether any part of `TEXT_DRAW` is worth separating without touching its public layout semantics. |
...
1. Derived render-type next-family audit: `TEXT_DRAW` / `FONT_DRAW`, and only take it if a similarly isolated low-risk slice exists.
