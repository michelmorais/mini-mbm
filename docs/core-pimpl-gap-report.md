...
| `include/core_mbm/texture-manager.h` / `TEXTURE`, TTF helpers | `TEXTURE_MANAGER` internals and `TEXTURE` backend handle are hidden. `TEXTURE` now exposes `hasAlphaChannel()` / `setAlphaChannelEnabled()` and repo call sites use those accessors, but `useAlphaChannel` storage remains in the public class for compatibility during this prep stage. TTF methods still expose `stbtt_aligned_quad` in public signatures. | `useAlphaChannel` is a simple asset property, so accessor prep is low-risk; the actual storage move is a separate compatibility decision. `stbtt_aligned_quad` is a third-party type leak, but not a backend/OS leak. | Medium. Texture/font code is used widely by Lua, editor, and render types. | Accessor prep is done. The next isolated milestone can decide whether to move `useAlphaChannel` storage behind private state or leave it public, and the TTF API decision remains separate. |
...
1. TTF public API decision for `stbtt_aligned_quad`: keep as accepted third-party API or wrap it.
2. `CAMERA_TARGET` accessor prep only. Do not move storage until Lua/editor compatibility is explicitly accepted.
3. `MESH_MBM` / `MESH_MBM_DEBUG` read-only compatibility audit. Do not edit storage in the audit milestone.
