...
1. Optional `BUFFER_MESH` accessor prep only for narrow read helpers such as render-buffer handle and subset access. Do not move storage in that milestone.
2. Optional `MESH_MBM` follow-up for remaining focused `infoPhysics` writes in shape/build paths only if the extra helper surface is clearly worth it.
3. Optional `MESH_MBM_DEBUG` owner-side algorithm helper review only if continuing this class is still worth the compatibility surface.
