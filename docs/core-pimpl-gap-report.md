...
| `include/core_mbm/mesh-manager.h` / `MESH_MBM`, `MESH_MBM_DEBUG`, `BUFFER_MESH` | `MESH_MANAGER` cache/fake-release state is behind `Impl`, but mesh file/debug structs still expose large public layouts. `MESH_MBM` now has accessor prep for repeated physics/animation metadata reads through `getPhysicsInfo()`, `getAnimationInfo()`, `getTotalAnimations()`, and `getAnimationHeader(index)`, plus narrow physics-write helpers for reset/cube/sphere/cube-complex/triangle writes used by shape builders and debug cloning. `BUFFER_MESH` now also has narrow read helpers for render-buffer lookup, loaded-buffer checks, subset count, and subset lookup, and the main render/animation/backend-debug read paths use them. `MESH_MBM_DEBUG` now has Lua-focused accessor prep for mesh kind, synchronized transform metadata, draw modes, detail-payload ownership, animation/effect metadata list access, raw frame/subset lookup helpers, and typed raw geometry-array helpers, while `BUFFER_MESH_DEBUG` storage layout remains direct. The owner-side mesh-debug algorithm review and the `BUFFER_MESH` storage-move review are both done and were intentionally not turned into helper churn. | These are file-format/editor/debug data contracts, not OS/backend SDK handles. The main risk is source compatibility across Lua/editor/debug paths rather than backend leakage. `MESH_MBM` is now in accessor-prep for metadata reads plus the currently targeted owned-physics write families; legacy/load parsing still uses `INFO_PHYSICS &` locals and is not yet a storage relocation candidate. `BUFFER_MESH` is now in accessor-prep for read paths but not storage relocation because construction/factory paths and debug extraction still depend on the aggregate layout; `MESH_MBM_DEBUG` is now in the accessor-prep stage for Lua metadata/detail, animation/effect, frame/subset lookup, and geometry-array access paths, but not for storage relocation. Owner-side edit/validation algorithms remain deliberately direct because they are the editing behavior, not just leaked storage reads. | High overall. `MESH_MBM_DEBUG` is still the broadest compatibility surface because public `BUFFER_MESH_DEBUG` layout and the editing algorithms both depend on raw arrays even after the Lua helper slices. `MESH_MBM` drops further for the currently targeted owned-physics write paths, but legacy/load/debug parser-local population still uses direct `INFO_PHYSICS` container writes. `BUFFER_MESH` stays medium for storage moves because runtime load/factory code and debug extraction still reach directly into both `BUFFER_MESH` and nested `BUFFER_GL` layout. | `MESH_MBM` metadata and the current owned-physics helper slices are done; if continued, the next audit should ask whether parser-local `INFO_PHYSICS` population is worth abstracting at all. `BUFFER_MESH` read-helper slice and storage-move audit are done; revisit only with a larger runtime-mesh builder/debug-extraction redesign. For `MESH_MBM_DEBUG`, Lua metadata/detail, animation/effect, frame/subset lookup, geometry-array helper slices, and the owner-side algorithm audit are done. If revisited, do it only for a more structural debug-mesh editing redesign, not for another isolated helper pass. |
...
1. Optional final `MESH_MBM` physics audit: decide whether parser-local `INFO_PHYSICS` population in load/legacy helpers is worth abstracting further, or should remain intentionally direct.
2. Only after that, consider a dedicated accessor-prep milestone for `TEXT_DRAW` / `FONT_DRAW` cross-owner and Lua-facing fields. Do not pair it with a storage move.
3. Revisit `BUFFER_MESH` only if a larger runtime-mesh builder/debug-extraction redesign becomes worth the compatibility cost.
4. Revisit `MESH_MBM_DEBUG` only if a larger debug-mesh editing/storage redesign becomes worth the compatibility cost.
