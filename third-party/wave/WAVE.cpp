...
void WaveFile::SetupFormat(int sampleRate, short bitsPerChannel, short channels)
{
	...
	SetBytesPerSample((uint16_t)((bitsPerChannel >> 3) * channels));
	...
	try {
		...
		try {
			...
			while (bytesRead < other.dataLength) {
				...
				size_t bytesToRead = (size_t) std::min(transferBufSize,size_t(other.dataLength - bytesRead));
...
bool WaveFile::ReadSample(float& sample)
{
	...
	sample = static_cast<float>(fSample);
...
bool WaveFile::ReadRaw(char* buffer, size_t numBytes,size_t & iTotal_read)
{
	iTotal_read = fread(buffer, 1, numBytes, GetFile());
	return iTotal_read > 0;
}
